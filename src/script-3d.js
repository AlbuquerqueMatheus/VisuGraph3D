...
function generateSceneCode(scene, camera, renderer) {
  ...
  scene.children.forEach((object) => {
    if (object.isMesh) {
      ...
      code += `
   const geometry = new THREE.${object.geometry.type}(${Object.values(
   ...
   mesh.position.set(${object.position.x}, ${object.position.y}, ${
        object.position.z
      });
   scene.add(mesh);
   `;
      ...
      // Código para luzes
      code += `
   const light = new THREE.${object.type}(${object.color.getHex()}, ${
        object.intensity
      });
   light.position.set(${object.position.x}, ${object.position.y}, ${
        object.position.z
      });
   scene.add(light);
   `;
...
const loadTextureAsync = (url) => {
  return new Promise((resolve, reject) => {
    textureLoader.load(
      url,
      (texture) => {
        texture.minFilter = THREE.LinearFilter; // Filtro mais leve
        texture.generateMipmaps = false; // Desativa geração de mipmaps
        resolve(texture);
      },
      undefined,
      () => {
...
const loadTexturesForMaterial = async (type) => {
  if (type === "metal") {
    const [color, displacement, metalness, normal, roughness] =
      await Promise.all([
    ...
    const [color, displacement, normal, roughness, ambientOcclusion] =
      await Promise.all([
    ...
    const [color, displacement, normal, ambientOcclusion, roughness] =
      await Promise.all([
...
window.addEventListener("load", () => {
  ["metal", "wood", "pedra", "moss"].forEach(async (type) => {
    if (!textures[type].color) {
      await loadTexturesForMaterial(type);
...
cubeAppearanceFolder
  ...
  .onChange(async (value) => {
    if (value !== "none" && !textures[value].color) {
      await loadTexturesForMaterial(value);
    }
...
animationsFolder
  ...
  .onChange(() => {
    ...
    for (let i = 0; i < vertices.length; i += 3) {
      ...
      vertices[i] =
        originalX + Math.sign(originalX) * debugObject.extrudeAmount;
      vertices[i + 1] =
        originalY + Math.sign(originalY) * debugObject.extrudeAmount;
      vertices[i + 2] =
        originalZ + Math.sign(originalZ) * debugObject.extrudeAmount;
    }
...
const applyArrayModifier = () => {
  ...
  for (let i = 1; i < arrayModifier.count; i++) {
    ...
    newMesh.position.set(
      arrayModifier.offsetX * i,
      arrayModifier.offsetY * i,
      arrayModifier.offsetZ * i
    );
    scene.add(newMesh);
  }
};
...
arrayModifierFolder
  .add(arrayModifier, "count")
  .min(1)
  .max(10)
  .step(1)
  .name("Count")
  .onChange(applyArrayModifier);
arrayModifierFolder
  .add(arrayModifier, "offsetX")
  .min(-2)
  .max(2)
  .step(0.1)
  .name("Offset X")
...
transformControlsFolder
  .add(transformModes, "mode", ["none", "translate", "rotate", "scale"])
  .name("Mode")
